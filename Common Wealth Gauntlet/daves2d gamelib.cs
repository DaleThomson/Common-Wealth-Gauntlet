...
namespace gamelib2d
{
    public class Camera2D
    {
        ...
        public Camera2D(Viewport viewport, int gwidth, int gheight)
        {
            _viewport = viewport;
        ...
        public void setcam(Vector2 character_position)
        {
            Viewport viewp = _viewport;
            viewp.Width = (int)(viewp.Width * (1 / Zoom));
            viewp.Height = (int)(viewp.Height * (1 / Zoom));

            if (character_position.X >= viewp.Width / 2 && character_position.X <= gamewidth - viewp.Width / 2)
            {
                position.X = character_position.X;
            }
            else
            {
                if (character_position.X < viewp.Width / 2)
                {
                    position.X = viewp.Width / 2;
                }
                else
                {
                    position.X = gamewidth - viewp.Width / 2;
                }
            }

            if (character_position.Y >= viewp.Height / 2 && character_position.Y <= gameheight - viewp.Height / 2)
            {
                position.Y = character_position.Y;
            }
            else
            {
                if (character_position.Y < viewp.Height / 2)
                {
                    position.Y = viewp.Height / 2;
                }
                else
                {
                    position.Y = gameheight - viewp.Height / 2;
                }
            }
        }
    ...
    */
    ...
    public class sprite2d
    {
        ...
        public float rotspeed = 0;          // Speed they should spin at
        ...
        public void automove2(int leftlimit, int rightlimit, int toplimit, int bottomlimit)
        {
            oldposition = position;
            position += velocity; // Add current velocity to the position of the sprite
            movement = position - oldposition;

            // Check if the sprite hits any of the four sides and bounce it off them
            if ((position.X + rect.Width / 2) >= rightlimit)
            {
                velocity.X = -velocity.X;
                position.X = rightlimit - rect.Width / 2;
            }
            if ((position.X - rect.Width / 2) <= leftlimit)
            {
                velocity.X = -velocity.X;
                position.X = leftlimit + rect.Width / 2;
            }
            if ((position.Y + rect.Height / 2) >= bottomlimit)
            {
                velocity.Y = -velocity.Y;
                position.Y = bottomlimit - rect.Height / 2;
            }
            if ((position.Y - rect.Height / 2) <= toplimit)
            {
                velocity.Y = -velocity.Y;
                position.Y = toplimit + rect.Height / 2;
            }
            updateobject();
        }
        ...
        // Use this method to draw the image at a specified position
        public void drawmeoffset(ref SpriteBatch sbatch, Vector3 offset)
        {
            if (visible)
            {
                Vector3 temppos = position - offset;

                Rectangle newrect = rect;
                newrect.X = (int)temppos.X;
                newrect.Y = (int)temppos.Y;
    ...
    public class animatedsprite
    {
        ...
        public float rotation = 0;              // Rotation amount
        public float fpsfactor = 1f;          // Factor by which animation frame rate relates to player speed
        public int Xboundingreduce = 0;         // Reduces bounding on the X=Axis
        public int Yboundingreduce = 0;         // Reduces bouding on the Y-Axis
        private Boolean ButtonpressedA = false; // Set Button A to not pressed
        private Boolean ButtonpressedB = false; // Set Button B to not pressed
        private int pushedlast = 1;             // Set pushed last to one
        public float RunningTime;               // Declare RunningTime
        public int FinishingPosition = 0;       // Set Finishing Position to 0
        public int CharacterSelection = 1;      // Set Character Selection to 1
        public Boolean selected = false;        // No characters selected.

        public animatedsprite() { }             // Empty constructor to avoid crashes

        // Constructor to set initial position and old position
        public animatedsprite(Vector3 pos, float frictiontemp, float gravitytemp, float characterspeed, int numberofanimations, float fps_ratio)
        {
            ...
            fpsfactor = fps_ratio;      // Frames per second ratio (set this to zero if you want a constant frame rate)
        }

        // Code for player movement
        public void dalemove(GamePadState pad, float gtime, int gwidth, int gheight, SoundEffect SFX)
        {
            if (pad.Triggers.Left > 0.1f && !ButtonpressedA && !ButtonpressedB && pushedlast == 1) // Checks that the left trigger is pressed with enough force and that A & B have not been pressed and that pushed last equals 1
            {
                velocity.X += power;   //Sets Velcoity.
                ButtonpressedA = true; // Sets Button A to being pressed.
                pushedlast = 0; // Sets pushedlast to 0.
                SFX.Play(); // Plays a sound effect when pressed
            }

           if (pad.Triggers.Left < 0.1f) //Checks that force is no longer being applied to left trigger.
            {
                ButtonpressedA = false; //Sets Button A to not being pressed.
            }

            if (pad.Triggers.Right > 0.1f && !ButtonpressedA && !ButtonpressedB && pushedlast == 0) // Checks that the right trigger is pressed with enough force and that A & B have not been pressed and that pushed last equals 0
            {
                velocity.X += power;   // Set velocity based on direction stick is pressed in
                ButtonpressedB = true; // Makes it so that the last button pressed is set to true
                pushedlast = 1; // Sets pushed last to 1
                SFX.Play(); // Plays a sound effect when pressed
            }

            if (pad.Triggers.Right < 0.1f) // Checks that force is no longer being applied to right trigger.
            {
                ButtonpressedB = false; //Sets Button B to not being pressed.
            }
        ...
        public void updatesprite(float gtime, int dwidth, int groundlevel)
        {
            if (visible)
            {
                ...
                if (fpsfactor > 0)
                {
                    ...
                    if (state < 2)
                        spriteanimation[state].framespersecond = (int)Math.Round(Math.Abs(velocity.X * fpsfactor), 0);
                ...
                if (position.Y >= groundlevel - spriteanimation[state].rect.Height / 2 + Yboundingreduce)
                {
                    position.Y = groundlevel - spriteanimation[state].rect.Height / 2 + Yboundingreduce;
                    velocity.Y = 0;
                    inair = false;
                }
        ...
        {
            bbox = new BoundingBox(new Vector3(position.X - spriteanimation[state].rect.Width / 2 + Xboundingreduce, position.Y - spriteanimation[state].rect.Height / 2 + Yboundingreduce, 0),
                new Vector3(position.X + spriteanimation[state].rect.Width / 2 - +Xboundingreduce, position.Y + spriteanimation[state].rect.Height / 2 - Yboundingreduce, 10));
        }
        ...
        // Make the sprite jump based on the forceapplied which is passed in
        public void forcejump(float forceappplied, float gtime, int gwidth, int gheight)
        {
            // Set direction and force of the jump, taking account of the speed of the pc
            velocity.Y -= forceappplied;
            ...
            updatesprite(gtime, gwidth, gheight);
        }



        public Boolean checkforcollisions(List<sprite2d> obstacles)
        {
            ...
            {
                if (obstacles[i].visible && bbox.Intersects(obstacles[i].bbox))
                {
                    collision = true;
                    updatebox();

                    // If dude hits left or right side of platform bounce him back
                    if ((bboxold.Max.X < obstacles[i].bbox.Min.X || bboxold.Min.X > obstacles[i].bbox.Max.X)
                        && bboxold.Max.Y > obstacles[i].bbox.Min.Y && bboxold.Min.Y < obstacles[i].bbox.Max.Y)
                    {
                        ...
                        else if (bbox.Min.Y < obstacles[i].bbox.Min.Y)
                        {
                            ...
                            velocity.Y = 0;
                            position += obstacles[i].movement;
                            position.Y = obstacles[i].bbox.Min.Y - spriteanimation[state].rect.Height / 2 + Yboundingreduce;
                        }
                }
            }
        ...
        {
            if (visible)
                spriteanimation[state].drawme(ref sbatch, position);
        }
    ...
    public static class sfunctions2d
    {
        ...
        public static Boolean overlaps(Texture2D image1, Rectangle box1, Texture2D image2, Rectangle box2)
        {
            Boolean collision = false;

            if (box1.Intersects(box2))
            {
                int arraysize1 = image1.Width * image1.Height;
                // Declare an array to hold the pixel data 
                uint[] colourdata1 = new uint[arraysize1];
                // Populate the array 
                image1.GetData(colourdata1, 0, arraysize1);

                int arraysize2 = image2.Width * image2.Height;
                // Declare an array to hold the pixel data 
                uint[] colourdata2 = new uint[arraysize2];
                // Populate the array 
                image2.GetData(colourdata2, 0, arraysize2);
